<template>
        ...
        <g id="Layer_x0020_1">
          ...
          <g clip-path="url(#id0)">
            <image
              ...
              width="10945.33"
              height="11310.17"
              xlink:href="Step-one_Images\Step-one_ImgID1.png"
            />
          </g>
          <path
            fill="none"
  ...
  <div class="w-80 mx-auto h-full mt-20 pt-5 mb-28">
    <div class="mb-4">
      <label
      ...
      <SelectCity
        @childData="saveToStore('loadingPlace', $event)"
        id="loadingPlace"
      />
      <p
        v-if="loadingPlaceAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        محل بارگیری را انتخاب کنید *
      </p>
    </div>
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('loadingAddress', $event)"
        id="loadingAddress"
      />
      <p
        v-if="loadingAddressAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        آدرس دقیق محل بارگیری را وارد کنید *
      </p>
    </div>
    ...
    <div class="mb-4">
      ...
      <SelectCity
        id="dischargePlace"
        @childData="saveToStore('dischargePlace', $event)"
      />
      <p
        v-if="dischargePlaceAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        محل تخلیه بار را انتخاب کنید *
      </p>
    </div>
    <div class="mb-4">
      <label
        class="block text-gray-700 text-sm font-bold text-right"
        for="dischargeAddress"
      >
        آدرس دقیق محل تخلیه بار
      </label>
      <InputText
        type="text"
        @childData="saveToStore('dischargeAddress', $event)"
        id="dischargeAddress"
      />
      <p
        v-if="dischargeAddressAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        آدرس دقیق محل تخلیه بار را انتخاب کنید *
      </p>
    </div>
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('recipientName', $event)"
        id="recipientName"
      />
      <p
        v-if="recipientNameAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        نام و نام خانوادگی تحویل گیرنده را وارد کنید *
      </p>
    </div>
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('recipientMobile', $event)"
        id="recipientMobile"
      />
      <p
        v-if="recipientMobileAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        شماره تلفن تحویل گیرنده را وارد کنید *
      </p>
    </div>

    <Button title="مرحله بعد" v-on:click.prevent="validate" class="mt-8" />
  </div>
  <BottomNavigation />
</template>
...
import SelectCity from "../fields/SelectCity.vue";

export default {
  data() {
    return {
      loadingPlaceAlert: false,
      loadingAddressAlert: false,
      dischargePlaceAlert: false,
      dischargeAddressAlert: false,
      recipientNameAlert: false,
      recipientMobileAlert: false,
    };
  },
  ...
  methods: {
    validate() {
      if (
        this.$store.state.loading.loadingPlace === "" ||
        this.$store.state.loading.loadingPlace === "شهر مورد نظر را انتخاب کنید"
      ) {
        this.loadingPlaceAlert = true;
        setTimeout(() => {
          this.loadingPlaceAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.loadingAddress === "") {
        this.loadingAddressAlert = true;
        setTimeout(() => {
          this.loadingAddressAlert = false;
        }, 1800);
      } else if (
        this.$store.state.loading.dischargePlace === "" ||
        this.$store.state.loading.dischargePlace ===
          "شهر مورد نظر را انتخاب کنید"
      ) {
        this.dischargePlaceAlert = true;
        setTimeout(() => {
          this.dischargePlaceAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.dischargeAddress === "") {
        this.dischargeAddressAlert = true;
        setTimeout(() => {
          this.dischargeAddressAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.recipientName === "") {
        this.recipientNameAlert = true;
        setTimeout(() => {
          this.recipientNameAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.recipientMobile === "") {
        this.recipientMobileAlert = true;
        setTimeout(() => {
          this.recipientMobileAlert = false;
        }, 1800);
      } else {
        this.$router.push("/owner/new-order/step-two");
      }
    },

    saveToStore(input, event) {
      if (input === "loadingPlace")
        this.$store.state.loading.loadingPlace = event;
      if (input === "loadingAddress")
        this.$store.state.loading.loadingAddress = event;
      if (input === "dischargePlace")
        this.$store.state.loading.dischargePlace = event;
      if (input === "dischargeAddress")
        this.$store.state.loading.dischargeAddress = event;
      if (input === "recipientName")
        this.$store.state.loading.recipientName = event;
      if (input === "recipientMobile")
        this.$store.state.loading.recipientMobile = event;
    },
  },
};
</script>
