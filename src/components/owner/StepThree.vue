<template>
        ...
        <g id="Layer_x0020_1">
          ...
          <g clip-path="url(#id0)">
            <image
              ...
              width="10945.33"
              height="11310.17"
              xlink:href="Step-three_Images\Step-three_ImgID1.png"
            />
          </g>
          <path
            fill="none"
  ...
  <div class="w-80 mx-auto h-full mt-20 pt-5 mb-20">
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('bill', $event)"
        id="bill"
      />
      <p v-if="billAlert === true" class="text-right text-xs text-red-500 mt-2">
        بارنامه را مشخص کنید *
      </p>
    </div>
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('paymentType', $event)"
        id="paymentType"
      />
      <p
        v-if="paymentTypeAlert === true"
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('currencyType', $event)"
        id="currencyType"
      />
      <p
        v-if="currencyTypeAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        نوع ارز را مشخص کنید *
      </p>
    </div>
    <div class="mb-4">
      <label
      ...
      <InputText
        type="text"
        @childData="saveToStore('insurance', $event)"
        id="insurance"
      />
      <p
        v-if="insuranceAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        بیمه را مشخص کنید *
      </p>
    ...
    <div class="mb-4">
      ...
      <InputText
        type="text"
        @childData="saveToStore('price', $event)"
        id="price"
      />
      <p
        v-if="priceAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        قیمت را مشخص کنید *
      </p>
    ...
    >
      ...
      <CheckBox
        class="text-right mt-3 mb-1"
        label="می پذیرم"
        style="padding: 0 !important"
        @childData="setCheck($event)"
      />
      <p
        v-if="checkedAlert === true"
        class="text-right text-xs text-red-500 mt-2"
      >
        می پذیرم را انتخاب کنید *
      </p>
    </div>

    <Button title="ثبت بار" v-on:click.prevent="validate" class="mb-10" />
  </div>
  <BottomNavigation />
</template>
...
export default {
  data() {
    return {
      billAlert: false,
      paymentTypeAlert: false,
      currencyTypeAlert: false,
  ...
  methods: {
    ...
    validate() {
      if (this.$store.state.loading.bill === "") {
        this.billAlert = true;
        setTimeout(() => {
          this.billAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.paymentType === "") {
        this.paymentTypeAlert = true;
        setTimeout(() => {
          this.paymentTypeAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.currencyType === "") {
        this.currencyTypeAlert = true;
        setTimeout(() => {
          this.currencyTypeAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.insurance === "") {
        this.insuranceAlert = true;
        setTimeout(() => {
          this.insuranceAlert = false;
        }, 1800);
      } else if (this.$store.state.loading.price === "") {
        this.priceAlert = true;
        setTimeout(() => {
          this.priceAlert = false;
        }, 1800);
      } else if (this.checked === false) {
        this.checkedAlert = true;

        setTimeout(() => {
          this.checkedAlert = false;
        }, 1800);
      } else {
        this.$router.push("/owner/new-order/success");
      }
    },

    saveToStore(input, event) {
      if (input === "bill") this.$store.state.loading.bill = event;
      if (input === "paymentType")
        this.$store.state.loading.paymentType = event;
      if (input === "currencyType")
        this.$store.state.loading.currencyType = event;
      if (input === "insurance") this.$store.state.loading.insurance = event;
      if (input === "price") this.$store.state.loading.price = event;
    },
  },
};
</script>
